...
public partial class Amministrazione : System.Web.UI.Page
{
    ...
    protected void gvDistricts_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            /*TextBox txtQta = (TextBox)e.Row.Cells[3].FindControl("txtQta");
    ...
    public void tabella()
    {
        ...
        while(rs.Read())
        {
            ...
            rs["Codice"] + "/"+rs["Taglie"]+" Cartellino " + rs["Prezzo_Cartellino"] + "€ \r\n Vendita "+rs["Prezzo_Vendita"] + "€",
        ...
        for (i = 0; i < righe; i++)
        {
            TextBox qtaAcquisto = (TextBox)grdArticoli.Rows[i].Cells[3].FindControl("txtQta");
            qtaAcquisto.Text = "0";
        }
    ...
    public void tabella(string Descr, string Marca, string Cat, string PrezzoDa, string PrezzoA, string Cod, articoli[] listaarticoli)
    {
        try
        {
            ...
            for (i = 0; i < righe; i++)
            {
                ...
                {
                    if (art.GetCodice() == grdArticoli.Rows[i].Cells[1].Text.Substring(0, grdArticoli.Rows[i].Cells[1].Text.IndexOf("/")).Trim())
                    {
                                trovato = true;
    ...
    public void tabella(articoli[] listaarticoli)
    {
        ...
        foreach (articoli art in listaarticoli)
        {
            ...
            art.GetCodice()+"/"+art.GetTaglia()+" Cartellino "+art.GetPrezzo_C() + " € \n\r Vendita"+art.GetPrezzo() + " €",
            art.GetQtaMag(),
            string.Empty);
        }
        grdArticoli.DataSource = dt;
    ...
    protected void btnOrdina_Click(object sender, EventArgs e)
    {
        try
        {
           ...
           /* SmtpClient client = new SmtpClient("smtp.gmail.com", 587);
            ...
            Response.AppendHeader("Refresh", "0;url=OrdiniMoblie1.aspx");
        }
        catch
        {
       ...
       // Response.Write("oiwhg");
        ...
        if (int.TryParse(thisTextBox.Text, out num))
        {
            if (int.Parse(thisTextBox.Text) >= 0)
            {
                ...
                {
                    if (listaarticoli[i].GetCodice() == thisGridViewRow.Cells[1].Text.Substring(0, thisGridViewRow.Cells[1].Text.IndexOf("/")).Trim())
                    {
                            trovato = true;
                    }
                    if (!trovato)
                ...
                {
                    thisTextBox.BackColor=System.Drawing.Color.White;
                    thisTextBox.BorderWidth = 1;
                    thisTextBox.BorderColor = System.Drawing.Color.Black;
                    ...
                    thisGridViewRow.Cells[2].Text = listaarticoli[i].GetQtaMag().ToString();
                    //update1.Update();
                }
                else
                {
                    thisTextBox.BackColor = System.Drawing.Color.FromArgb(237, 67, 55);
                    thisTextBox.BorderWidth = 3;
                    thisTextBox.BorderColor = System.Drawing.Color.Red;
                }
            }
        }
        else
